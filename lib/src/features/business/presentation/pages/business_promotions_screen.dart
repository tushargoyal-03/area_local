...
class _BusinessPromotionsScreenState extends State<BusinessPromotionsScreen> with SingleTickerProviderStateMixin {
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 12.h),
              AppTextField(controller: discountCtrl, hint: 'Discount Code (Optional)'),
            ],
          ),
        ),
        actions: [
            ...
            onPressed: () {
              if (nameCtrl.text.isNotEmpty && titleCtrl.text.isNotEmpty) {
                ...
                context.read<BusinessBloc>().add(CreatePromotionRequested(
                  ...
                  discountCode: discountCtrl.text.isEmpty ? null : discountCtrl.text,
  ...
  Widget build(BuildContext context) {
    ...
    final isOwner = context.select((SessionBloc b) => b.state.user?.role == 'BusinessOwner');
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<BusinessBloc, BusinessState>(
      builder: (context, state) {
        ...
        if (state.error != null && state.nearbyPromotions.isEmpty) return Center(child: Text(state.error!));
        if (state.nearbyPromotions.isEmpty) return const Center(child: Text('No nearby promotions'));
...
class _MyPromotionsList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<BusinessBloc, BusinessState>(
      builder: (context, state) {
        if (state.isLoadingMine) return const Center(child: CircularProgressIndicator());
        if (state.error != null && state.myPromotions.isEmpty) return Center(child: Text(state.error!));
        if (state.myPromotions.isEmpty) return const Center(child: Text('You have no active promotions'));
...
Widget _buildPromoCard(Map<String, dynamic> promo, ColorScheme cs, TextTheme tt) {
  return Container(
